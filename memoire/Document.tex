...
\usepackage{booktabs} % Règles de tableaux \toprule \midrule \bottomrule / professional table rules
%%
...
\usepackage{svg} % Support des images SVG (requiert inkscape) / SVG support (requires inkscape)
\usepackage{dirtree}
\usepackage{float} % placement [H] : figures non flottantes, placées exactement où elles sont écrites / [H] non-floating placement
\usepackage{tikz} % schémas vectoriels (architecture du réseau) / vector diagrams
\usepackage{enumitem}
\usetikzlibrary{arrows.meta, positioning, calc, fit, backgrounds} % Stealth, right=.. of .., ($..!0.5!..$), fit/backgrounds : groupes asynchrones
...
\mainmatter\acresetall\include{chapters/5-Introduction}       % Introduction au sujet de recherche.
\include{chapters/6-Revue_litterature}  % Revue de littérature / Literature review
\include{chapters/7-Theme1}             % Premier thème (Doctorat) ou "Détails de la Solution" (Maîtrise) / First topic (PhD) or "Details of the Solution" (Master's).
\include{chapters/7-Theme2}             % Second thème (Doctorat) ou "Résultats théoriques et expérimentaux" (Maîtrise) / Second theme (PhD) or "Theoretical and experimental results" (Master's)
\include{chapters/7-Theme3}             % Troisième thème (Doctorat) ou effacez ce fichier si vous êtes à la Maîtrise / Third topic (PhD) or delete this file if you are in the Master's program
